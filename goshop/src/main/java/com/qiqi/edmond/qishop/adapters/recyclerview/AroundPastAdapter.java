...
public class AroundPastAdapter extends BaseFooterAdapter<AroundPastInfo> {
    ...
    public AroundPastAdapter(Context context, List<AroundPastInfo> data, View footer) {
        super(context, data);
        this.footer = footer;
    }
    ...
    public BaseItem initItem(ViewGroup parent) {
        View view = getInflater().inflate(R.layout.item_around_past, parent, false);
        return new MyItem(view);
    }

    @Override
    public void bindItem(BaseAdapter.BaseItem item, final int position) {
        ((MyItem) item).titleTextView.setText("df");
